...
public class StartupDataFixer {
    ...
    public void onStartup() {
        updateDefaultEmployeeNames();
        fixManagerAssignments();
        createSeedNotifications();
    }

    private void updateDefaultEmployeeNames() {
        log.info("StartupDataFixer: Updating default employee names to requested project names...");

        // RW001: Harsha Munjala
        employeeRepository.findById("RW001").ifPresent(e -> {
            if (e.getFirstName().equalsIgnoreCase("Rev") || e.getLastName().equalsIgnoreCase("Admin")) {
                e.setFirstName("Harsha");
                e.setLastName("Munjala");
                employeeRepository.save(e);
                log.info("StartupDataFixer: Updated RW001 to Harsha Munjala");
            }
        });

        // RW002: John Smith
        employeeRepository.findById("RW002").ifPresent(e -> {
            if (e.getFirstName().equalsIgnoreCase("Jane") || e.getLastName().equalsIgnoreCase("Manager")) {
                e.setFirstName("John");
                e.setLastName("Smith");
                employeeRepository.save(e);
                log.info("StartupDataFixer: Updated RW002 to John Smith");
            }
        });

        // RW003: Jack Wilson
        employeeRepository.findById("RW003").ifPresent(e -> {
            if (e.getFirstName().equalsIgnoreCase("John") || e.getLastName().equalsIgnoreCase("Employee")) {
                e.setFirstName("Jack");
                e.setLastName("Wilson");
                employeeRepository.save(e);
                log.info("StartupDataFixer: Updated RW003 to Jack Wilson");
            }
        });
    }
